...
import { fetchAMMInfo, fetchRecentTokens, fetchSortedPools, fetchPoolInfo, fetchTokenPrice, fetchTrendingTokens, searchTokens, fetchPoolTrades, fetchTokenBalance, fetchPools } from '../libs/fetches';
...
export const TokenDataContext = createContext<{
  ...
  usePoolList: ( duration: string, skip: number, limit: number, sort: string, direction?: string, refetchInterval?: number) => UseQueryResult<IPoolOverview[], Error>;
...
export const TokenDataProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchTokenTrades = async (address: string, txType: TRADE_TYPES, limit: number, offset: number): Promise<{ hasNext: boolean; nextCursor: number; items: ITokenTrading[] }> => {
    ...
    let tradeData: any = {}
    if (data) {
      tradeData.items = data.map((item: any) => {
        const newItem: ITokenTrading = {
          ...
          fromSymbol: item.from.symbol,
          toSymbol: item.to.symbol,
  ...
  const { data: SOL_PRICE } = useQuery<number>({
    queryKey: ['SOL_PRICE'],
    queryFn: async () => (await fetchTokenPrice(DEFAULT_TOKEN, USDC_TOKEN_ADDRESS)).price,
  });
  ...
  const usePoolList = ( duration: string, skip: number, limit: number, sort: string, direction: string = 'desc',  refetchInterval?: number) => {
    return useQuery<IPoolOverview[]>({
      queryKey: ['POOL_DETAIL', duration],
      queryFn: async () => (await fetchPools(duration, skip, limit, sort, direction)),
      refetchInterval: refetchInterval ? (refetchInterval || 0) * 60 * 1000 : false,
    });
  };

...
const TradeList = (props: Props) => {
  ...
  const [toSymbol, setToSymbol] = useState<string>('SOL')
  const [fromSymbol, setFromSymbol] = useState<string>('SOL')
  ...
  useEffect(() => {
    setData(history => {
      ...
      if (updatedData.length > 0) {
        setFromSymbol(updatedData[0].fromSymbol)
        setToSymbol(updatedData[0].toSymbol)
      }
    ...
    const listenerId = ws_pool?.registerListener('TXS_REALTIME_DATA', data => {
      if (data.type === 'TXS_REALTIME_DATA' && data.data && data.data.length > 0) {
        ...
        for (let tx of data.data) {
          ...
          to_symbol = newData.toSymbol
          from_symbol = newData.fromSymbol
          newData.volumeUSD = tx.volumeUSD || price * newData.amount;
          swapData.push(newData)
        }
        setToSymbol(to_symbol)
        setFromSymbol(from_symbol)

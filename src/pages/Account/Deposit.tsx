...
export default function Deposit() {
  ...
  const handleCopyDepositAddress = (event: React.MouseEvent<SVGElement>) => {
    if (userInfo) {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const trxId = await depositFunds(
  ...
  return (
    <div className={`flex flex-col gap-4 py-4`}>
      <span className="text-sm text-gray font-semibold">
        Deposit SOL to your GMI trading wallet.
      </span>
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="flex flex-col gap-2 w-full md:w-fit">
          ...
          <div className="flex items-center gap-1 border border-outline bg-[#FFFFFF10] px-4 py-2 rounded-lg justify-between">
            <span className="text-sm">
              {shortenAddress(userInfo?.deposit_wallet)}
            </span>
            <ClipboardDocumentIcon
        ...
        <div className="flex flex-col gap-2 items-end w-full md:w-fit">
          ...
          <Button className="primary w-full" onClick={handleDeposit}>
            Deposit
          </Button>
        </div>
      </div>

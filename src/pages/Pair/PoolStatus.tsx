...
export default function PoolStatus(props: Props) {
  ...
  const TYPE_6H = 2;
  const TYPE_24H = 3;
  const TYPE_LABEL = ["5M", "1H", "6H", "24H"];

  const [type, setType] = useState(TYPE_24H);
  ...
  const PoolStatusItem = ({ _type }: { _type: number }) => {
    const percent =
      _type === TYPE_5M
        ? formatPercent(props?.poolInfo?.priceChange5mPercent)
        : _type === TYPE_1H
        ? formatPercent(props?.poolInfo?.priceChange1hPercent)
        : _type === TYPE_6H
        ? formatPercent(props?.poolInfo?.priceChange6hPercent)
        : _type === TYPE_24H
        ? formatPercent(props?.poolInfo?.priceChange24hPercent)
        : "N/A";
    ...
    return (
      ...
      >
        <span className="text-xs">{TYPE_LABEL[_type]}</span>
        <span className="text-sm text-center w-full overflow-hidden text-ellipsis whitespace-nowrap font-bold">
          {percent}
        </span>
      </div>
    );
  ...
  return (
    <div className={`flex flex-col border border-border rounded-[20px] mx-2`}>
      ...
      <div className="flex flex-col items-center">
        <div className="grid grid-cols-[60px_1fr] w-full py-2">
          <div className="flex flex-col border-r border-border px-4 items-center justify-center -my-2">
